...
//   var html = " "; 
//   data.results[0].members.forEach(memberss=>{
//       html+= "<tr> <td>" + "<a href="+memberss.url
//            + ">"+ " "+  memberss.first_name 
//            + " "+ ( memberss.middle_name || "")
//            + " " + memberss.last_name+ "</a>" + "</td>"
//            + "<td>" + memberss.party + "</td>"+ "<td>"
//            + memberss.state + "</td>"+ "<td>" 
//            + memberss.seniority + "</td>"+ "<td>"
//            + memberss.votes_with_party_pct
//             + " " +  "%" + "</td>" + "</tr>"
//   });


  // ------------------NCreate Statistics Json------------------------

  let statistics= {

    "party": {
       "republicans":{"nro_reps": 0,
               "perc_voted_with": 0,            
    },
         "democrats":{"nro_reps": 0,
               "perc_voted_with": 0,            
     },
      "independents":{"nro_reps": 0,
               "perc_voted_with": 0,            
    }
  },
    "least_loyal_members":"",
    ...
    "most_loyal_members":"",
    "most_loyal_missed_votes": 0,
    "most_loyal_votes_against_pct": 0,

  };
  // ------------------get Array of Members per Party------------------------
      let repObject=[];
      let demObject=[];
      let indObject=[];
      let totality=[];
...
data.results[0].members.forEach(element => {
    if (element.party=="D") {
        demObject.push(element);
}   else if (element.party=="R") {
    repObject.push(element);
}    else{
    indObject.push(element);
}
 });     
 ...
 data.results[0].members.forEach(element => {
  ...
  totality.push(element)

});

  //console.log("Totality are ", totality);

 
  // ------------------Number of Members per Party------------------------

let numTotality=totality.length;

 let numRep=repObject.length;
//  console.log("Republicans are ",numRep);
 let numDem=demObject.length;
//  console.log("Democrats are ",numDem);
 let numInd=indObject.length;
//  console.log("Independents are ",numInd);


  // ------------------Save in Object Statistics: Number of Members per Party------------------------



    statistics.party.republicans.nro_reps =numRep;

    statistics.party.democrats.nro_reps =numDem;

    statistics.party.independents.nro_reps =numInd;
  ...
  // ------------------Average Function ------------------------
              ...
              function average(object) {
                let totalVotesWith=0;
                let averageWith=0;
                for (let i = 0; i < object.length; i++) {
                  totalVotesWith +=  object[i].votes_with_party_pct ;   
                  averageWith =  totalVotesWith/object.length;
                }
                return averageWith;
                    }
            ...
            // ------------------Save in Html Data Votes and Average ------------------------


          repVotes = Math.round(average(repObject));

          demVotes = Math.round(average(demObject));

          indVotes = Math.round(average(indObject));



          document.getElementById('repNumber').innerHTML = numRep.toString();
          document.getElementById('repVotes').innerHTML = repVotes.toString();
          
          document.getElementById('demNumber').innerHTML = numDem.toString();
          document.getElementById('demVotes').innerHTML = demVotes.toString();
          
          document.getElementById('inNumber').innerHTML = numInd.toString();
          document.getElementById('inVotes').innerHTML = indVotes.toString();
                    

   // ------------------ Getting Ten Percentage------------------------

  let RepPercent = Math.round(average(demObject));
  //console.log("Average Republican are ",RepPercent);

  let DemPercent = Math.round(average(repObject));
  // console.log("Average Democrats are ",DemPercent);

   let IndPercent = Math.round(average(indObject));
  // console.log("Average Independent are ",IndPercent);


     // ------------------Save in Object Statistics: Average per Party------------------------



      statistics.party.republicans.perc_voted_with = RepPercent + "%" ;
    
      statistics.party.democrats.perc_voted_with=DemPercent + "%" ;

      statistics.party.independents.perc_voted_with=IndPercent + "%" ;

  
   // ------------------Function to Order by Perc Loyalty------------------------

     function orderByLoyalty(a, b) {
   ...
   // ------------------Ordering by loyalty------------------------
     ...
     let orderedObject=totality.sort(orderByLoyalty);
     //console.log("Ordered is",  orderedObject);

     congress = Array.from(orderedObject);

   // ------------------get exact number per 10 percent of each party------------------------


        function tenPerc(object) {
          
          let tenPercent
           tenPercent= (object.length)/10;

           return tenPercent;
        }

        tenPercent= Math.round(tenPerc(orderedObject));

        // console.log("10% is ", tenPercent )

   // ------------------extract those ten percent Least Loyal------------------------


        function leastArray(orderedArray){
          let leastLoyal=[]
            for (let i = 0; i <tenPerc(orderedArray) ; i++) {
              leastLoyal.push(orderedArray[i]);
            }
            return leastLoyal;
        }


        let leastMembers = leastArray(orderedObject);



  // ------------------Functions to get array for Missed Votes------------------------

          function theMissedVotes(array) {
            let missedVotes=[]
            array.forEach(element => {
              missedVotes.push(element.missed_votes)
            });
            return missedVotes;
          }


  // ------------------Functions to get array for Perc of Missed Votes------------------------

          function theMissedVotesPct(array) {
            let missedVotesPct=[]
            array.forEach(element => {
              missedVotesPct.push(element.votes_against_party_pct)
            });
  ...
  // ------------------Creating Html Table with data------------------------


              function createTable(tableData, values){  
                ...
                let nameData=document.createTextNode(values.first_name + " "  + values.last_name );
                
                 tdData.appendChild(nameData);
                ...
                tdMissedVotes.appendChild(document.createTextNode((Math.round((values.total_votes*values.votes_with_party_pct)/100))));
                
                trData.appendChild(tdMissedVotes);




                let tdPctMissed=  document.createElement("td");

                tdPctMissed.appendChild(document.createTextNode(values.votes_with_party_pct));
                
                trData.appendChild(tdPctMissed);
   ...
   // ----------------------  Rendering Table data    ---------------------------------
              ...
              for (let i = ((congress.length-1)); i > ((congress.length)-tenPercent); i--) {

                createTable(document.getElementById("least-engaged"), congress[i]);
              ...
              for (let i = 0; i < tenPercent; i++) {
                ...
                createTable(document.getElementById("most-engaged"), congress[i]);
